...
for idx, row in df.iterrows():
    ...
    if os.path.exists(flux_file_path):
        ...
        if len(flux_0_01) > 0:
            ...
            df.at[idx, 'FluxLimit(1e-6)_tbin_0.01'] = f"{flux_value * 1e6:.4f}"
            df.at[idx, 'Fluence(1e-6)_tbin_0.01'] = f"{fluence_value * 1e6:.4f}"
        if len(flux_0_1) > 0:
            flux_value = flux_0_1[0]
            fluence_value = flux_value * 0.1
            df.at[idx, 'FluxLimit(1e-6)_tbin_0.1'] = f"{flux_value * 1e6:.4f}"
            df.at[idx, 'Fluence(1e-6)_tbin_0.1'] = f"{fluence_value * 1e6:.4f}"
        if len(flux_1_0) > 0:
            flux_value = flux_1_0[0]
            fluence_value = flux_value * 1.0
            df.at[idx, 'FluxLimit(1e-6)_tbin_1.0'] = f"{flux_value * 1e6:.4f}"
            df.at[idx, 'Fluence(1e-6)_tbin_1.0'] = f"{fluence_value * 1e6:.4f}"
